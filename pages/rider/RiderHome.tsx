...
const RiderHome: React.FC = () => {
  ...
  const Sidebar = () => (
    <div className="fixed inset-0 z-50 flex">
        <div className="w-80 bg-white shadow-2xl h-full flex flex-col animate-fade-in">
            <div className="p-6 bg-black text-white">
                <div className="flex items-center mb-6">
                    <img src={user?.avatarUrl} className="w-16 h-16 rounded-full border-2 border-white mr-4"/>
                    <div>
            ...
            <nav className="flex-grow p-4 space-y-1">
                <button onClick={() => setView('MAP')} className={`w-full flex items-center p-4 rounded-xl font-medium transition-colors ${view === 'MAP' ? 'bg-gray-100' : 'hover:bg-gray-50'}`}>
                    <MapPin size={20} className="mr-3" /> Map
                </button>
                <button onClick={() => setView('WALLET')} className="w-full flex items-center p-4 rounded-xl font-medium hover:bg-gray-50 transition-colors">
                    <Wallet size={20} className="mr-3" /> Wallet
                </button>
                 <button onClick={() => setView('HISTORY')} className="w-full flex items-center p-4 rounded-xl font-medium hover:bg-gray-50 transition-colors">
                    <History size={20} className="mr-3" /> Your Trips
                </button>
                <button className="w-full flex items-center p-4 rounded-xl font-medium hover:bg-gray-50 transition-colors">
            ...
            <div className="p-4 border-t">
                <button onClick={handleLogout} className="w-full flex items-center p-4 text-red-600 font-bold hover:bg-red-50 rounded-xl">
                    <LogOut size={20} className="mr-3" /> Log Out
                </button>
            </div>
        </div>
        <div className="flex-grow bg-black/50 backdrop-blur-sm" onClick={() => setView('MAP')}></div>
    </div>
  );
      ...
      return (
          <div className="h-screen bg-gray-50 flex flex-col">
             <div className="bg-white p-6 shadow-sm flex items-center">
                 <button onClick={() => setView('MAP')} className="mr-4 p-2 hover:bg-gray-100 rounded-full"><X size={24}/></button>
             ...
             <div className="p-6">
                 <div className="bg-black text-white rounded-2xl p-8 mb-8 shadow-xl relative overflow-hidden">
                     <div className="relative z-10">
                         ...
                         <button onClick={() => addFunds(50)} className="bg-green-500 text-black px-6 py-3 rounded-full font-bold flex items-center hover:bg-green-400 transition-colors">
                             <Plus size={20} className="mr-2" /> Top Up $50
                         </button>
                     </div>
                     <div className="absolute -right-10 -bottom-20 w-64 h-64 bg-gray-800 rounded-full opacity-50 blur-3xl"></div>
                 </div>
                ...
                {user?.history?.map((trip) => (
                    <div key={trip.id} className="bg-white p-4 rounded-xl shadow-sm">
                        ...
                        <div className="flex justify-between items-center pt-3 border-t text-sm text-gray-500">
                            ...
                            <button className="text-blue-600 font-medium">Report Issue</button>
                        </div>
                    </div>
                ))}
  ...
  return (
    <div className="h-screen relative bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 z-30">
            <button onClick={() => setView('MENU')} className="bg-white p-3 rounded-full shadow-lg hover:bg-gray-50 transition-colors">
                <Menu size={24} />
            </button>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-30 bg-white rounded-t-3xl shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.3)] max-h-[80vh] overflow-y-auto transition-all">
                ...
                <div className="p-6">
                    ...
                    {!destination ? (
                        <div className="animate-fade-in">
                            ...
                            <div className="bg-gray-100 p-4 rounded-xl flex items-center mb-4 hover:bg-gray-200 transition-colors cursor-pointer">
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-2">
                                <button className="bg-gray-100 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-gray-200">Home</button>
                                <button className="bg-gray-100 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-gray-200">Work</button>
                                <button className="bg-gray-100 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap hover:bg-gray-200">Gym</button>
                            </div>
                        </div>
                    ) : (
                        /* Vehicle Selection */
                        <div className="animate-fade-in">
                            <div className="flex items-center mb-4">
                                <button onClick={() => setDestination("")} className="p-2 -ml-2 mr-2 hover:bg-gray-100 rounded-full"><X size={20}/></button>
                                <h3 className="font-bold text-lg">Choose a ride</h3>
                            </div>
                            
                            {/* Trip Info Summary */}
                            <div className="flex items-center justify-between bg-blue-50 p-3 rounded-lg mb-4 text-blue-900 text-sm">
                                <div className="flex items-center">
                            ...
                            <div className="space-y-2 mb-6">
                                {VEHICLE_OPTIONS.map(v => (
                                    <div 
                                        ...
                                        className={`flex items-center justify-between p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedVehicle.id === v.id ? 'border-black bg-gray-50' : 'border-transparent hover:bg-gray-50'}`}
                                    >
                                        <div className="flex items-center">
                            ...
                            <div className="flex items-center justify-between mb-4 px-1">
                                ...
                                <span className="text-blue-600 font-bold text-sm cursor-pointer">Switch</span>
                            </div>

                            <button onClick={handleBook} className="w-full bg-green-500 text-black font-bold py-4 rounded-xl text-lg hover:bg-green-400 shadow-lg transition-all transform active:scale-95">
                ...
                <div className="p-6 animate-slide-up">
                    {activeRide.status === RideStatus.SEARCHING ? (
                        <div className="text-center py-4">
                            ...
                            <h2 className="text-lg font-bold">Connecting to nearby drivers...</h2>
                            <button onClick={cancelRide} className="mt-4 text-red-500 font-bold text-sm">Cancel Request</button>
                        </div>
                    ) : (
                        <>
                            <div className="flex items-center justify-between mb-6 border-b pb-6">
                                <div className="flex items-center">
                                     <img src={MOCK_DRIVER.avatarUrl} className="w-16 h-16 rounded-full border-4 border-gray-100 shadow-md mr-4" />
                                     <div>
                                ...
                                <div className="flex flex-col gap-2">
                                     <button className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-black hover:bg-gray-200"><Phone size={20}/></button>
                                     <button className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-black hover:bg-gray-200"><MessageSquare size={20}/></button>
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-green-600">{activeRide.status.replace('_', ' ')}</p>
                                </div>
            ...
            <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in">
                <div className="bg-white w-full max-w-sm rounded-3xl p-8 text-center shadow-2xl">
                    ...
                    <div className="flex justify-center space-x-2 mb-8">
                        {[1,2,3,4,5].map(star => (
                            <Star key={star} size={36} className="text-gray-200 hover:text-yellow-400 hover:fill-yellow-400 cursor-pointer transition-colors" />
                        ))}
                    </div>
                    ...
                    <button onClick={handleRate} className="w-full bg-green-500 text-black font-bold py-4 rounded-xl text-lg hover:bg-green-400">Submit</button>
                </div>
            </div>
        )}
    </div>
